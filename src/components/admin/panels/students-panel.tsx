...
export function StudentsPanel() {
  ...
  const openEdit = (s: Student) => { setEditing(s); setForm({ nisn: s.nisn || '', nis: s.nis || '', name: s.name, photo: s.photo || '', birthDate: s.birthDate || '', gender: s.gender, className: s.className, academicYear: s.academicYear, status: s.status }); setOpen(true); };
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((s, i) => (
                    <tr key={s.id} className={`border-b border-border/50 hover:bg-primary/5 transition-colors ${i % 2 === 1 ? 'bg-muted/30' : 'bg-background'}`}>
                      ...
                      <td className="p-3 hidden lg:table-cell text-xs text-muted-foreground">{s.birthDate || '-'}</td>
                      <td className="p-3 text-center">
